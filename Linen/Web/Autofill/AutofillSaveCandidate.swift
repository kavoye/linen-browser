...
nonisolated enum AutofillSaveCandidate: Sendable {
    ...
    var identity: [String] {
        switch self {
        case .password(let login): [login.origin, login.username]
        case .card(let card): [card.number]
        case .contact: values
        }
    }
    ...
    var values: [String] {
        switch self {
        case .password(let login): [login.origin, login.username, login.password]
        case .card(let card): [card.number, card.cardholder, card.month.map(String.init) ?? "", card.year.map(String.init) ?? ""]
    ...
    var summary: String {
        switch self {
        case .password(let login): login.username.isEmpty ? login.origin : login.username
        case .card(let card): card.summary.label
